...
begin  
  ...
  WHERE ddips.avg_fragmentation_in_percent > 10.0 AND ddips.index_id > 0;'

  --print @exec_stmt; -- DEBUG
  exec (@exec_stmt)

  -- Declare the cursor for the list of partitions to be processed.
  DECLARE partitions CURSOR FOR SELECT * FROM #work_to_do where pagecount > 500;
  ...
  WHILE (1=1)
      BEGIN;
          FETCH NEXT
             FROM partitions
             INTO @objectid, @objectname, @indexid, @indexname, @schemaname, @partitionnum, @frag, @pagecount;
  ...
  -- 30 is an arbitrary decision point at which to switch between reorganizing and rebuilding.
          ...
          IF @frag >= 10.0
              SET @command = N'use ' +  quotename(@dbname, N'[') + ' ALTER INDEX ' + @indexname + N' ON ' + @schemaname + N'.' + @objectname + N' REBUILD';
          --IF @partitioncount > 1
