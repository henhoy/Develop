...
** 
** SVN tags:
** $Date: 2014-09-14 20:41:02 +0200 (sÃ¸, 14 sep 2014) $
** $Revision: 499 $
***********************************************************************************************/
...
/*** Create a global runtime trace table  ***/
/*** MiracleOnlineRuntimeTrace MT       ***/
create table ##MT (
stamp	datetime default getdate(),
trace	varchar(300)
);
insert into ##MT (trace) values ('Miracle Online tjek started')

/*** Create a few helper functions **/
/* Trace */
insert into ##MT (trace) values ('Creating Helper functions')
...
/* Trace */
insert into ##MT (trace) values ('Create MORLT Table')
...
insert into #MORLT values ('sqlserver_check_version','1.5.2');
...
/* Insert default minimum time to last good checkdb run*/
insert into #MORLT values ('checkdbThreshold','7');
/* Insert default maximum days since last completed Full backup*/
insert into #MORLT values ('MaxFullBackupThreshold','7');
-- not used yet -- /* Insert default maximum days since last completed Diff backup*/
-- not used yet -- insert into #MORLT values ('MaxDiffBackupThreshold','7'); 
/* Insert default maximum hour since last completed Transactionlog backup*/
insert into #MORLT values ('MaxTransactionlogBackupThreshold','25');
...
-- Added in revision 495
insert into #ignore_waits values ('SP_SERVER_DIAGNOSTICS_SLEEP');
insert into #ignore_waits values ('HADR_FILESTREAM_IOMGR_IOCOMPLETION');
insert into #ignore_waits values ('DIRTY_PAGE_POLL');
...
/*********************************************************************************************/
/* Trace */
insert into ##MT (trace) values ('Start collecting performance data')
...
/* Trace */
insert into ##MT (trace) values (' - Collecting period: ' + convert(varchar, @waittimes) + ' minute(s)')

while @waittimes > 0
begin 
  waitfor DELAY '000:01:00'   -- et minut
  set @waittimes = @waittimes - 1;
  insert into ##MT (trace) values (' -- Waited one minute - ' + convert(varchar, @waittimes) + ' minute(s) back ...')
end
...
/* Trace */
insert into ##MT (trace) values ('Finish collecting performance data')
...
/*********************************************************************************************/
/* Trace */
insert into ##MT (trace) values ('Collect instance information')
...
select '<tr><th>Platform</th><th>OS Version</th><th>Processors</th><th>Numa</th><th>Process ID</th><th>Phys. Mem MB</th><th>Max Mem MB</th><th>Min Mem MB</th><th>Cur. Mem MB</th>' + 
       '<th>Cur. Page life (sec)</th><th>Cur. Connections</th></tr>'
select '<tr>' +
...
/*********************************************************************************************/
/* Trace */
insert into ##MT (trace) values ('Collect uptime information')
...
/*********************************************************************************************/
/* Trace */
insert into ##MT (trace) values ('Collect CPU utilization')
...
/*********************************************************************************************/
/* Trace */
insert into ##MT (trace) values ('Collect job information')
...
select '<table>'
select '<tr><th>Job Name</th><th>Job status</th><th>Schedule name</th><th>Type</th><th>Status</th><th>Last runtime</th><th>Last run duration</th><th>Avg run duration</th><th>Last result</th><th>Overall (%)</th><th>Next runtime</th></tr>'
select '<tr>' +
       '<td>' + rtrim(sj.name) + '</td>' +
       '<td>' + case 
       ...
       '<td>' + isnull(sjsch.name, '-- No Schedule --') + '</td>' +
	   '<td>' + isnull(sjsch.freq_type, '-- n/a --') + '</td>' +
	   '<td>' + case 
                  when sjsch.enabled = 0 then 'Not Enabled'
                  when sjsch.enabled = 1 then 'Enabled'
				  else 'n/a'
                end + '</td>' + 
	   '<td class="aligncenter" >' + case
                  when sjsch.freq_type = 'Agent start' then 'Still Running ?'
	   ...
	   -- Calulate latest run duration
       '<td class="aligncenter" >' + case
                 when( last_run_duration = 0 )  then 
                   case 
                     when last_run_date = 0 then 'n/a' 
				     else ' < 1 sec'
				   end
				 when( last_run_duration < 100 )  then CONVERT(varchar,last_run_duration) + ' sec' 
				 when( last_run_duration < 6000 ) then 
				   convert(varchar,convert(int,reverse(substring(reverse(convert(varchar,last_run_duration)),1,4)))/100) + ' min'
				 when( last_run_duration > 6000 ) then 
				   convert(varchar,convert(int,reverse(substring(reverse(convert(varchar,last_run_duration)),5,50)))) + ' hr ' +
				   convert(varchar,convert(int,reverse(substring(reverse(convert(varchar,last_run_duration)),1,4)))/100) + ' min'
				 else ' - '
			   end + '</td>' +
   	   -- Calulate average run duration
       '<td class="aligncenter" >' + case
                 when runagg.avg_duration is null then 'No data'
				 when ( runagg.avg_duration = 0 )  then 
                   case 
                     when last_run_date = 0 then 'n/a' 
				     else ' < 1 sec'
				   end
				 when( runagg.avg_duration < 100 )  then CONVERT(varchar,runagg.avg_duration) + ' sec' 
				 when( runagg.avg_duration < 6000 ) then 
				   convert(varchar,convert(int,reverse(substring(reverse(convert(varchar,runagg.avg_duration)),1,4)))/100) + ' min'
				 when( runagg.avg_duration > 6000 ) then 
				   convert(varchar,convert(int,reverse(substring(reverse(convert(varchar,runagg.avg_duration)),5,50)))) + ' hr ' +
				   convert(varchar,convert(int,reverse(substring(reverse(convert(varchar,runagg.avg_duration)),1,4)))/100) + ' min'
				 else ' - '
			   end + '</td>' +		   
       '<td' + case 
...
left outer join (select sjs.schedule_id, sjs.job_id, sjs.next_run_date, sjs.next_run_time, 
                   isnull(ss.name, ' -- NULL -- ') as name, ss.enabled,
                   case 
                     when freq_type =   1 then 'One time'
                     when freq_type =   4 then 'Daily'
                     when freq_type =   8 then 'Weekly'
                     when freq_type =  16 then 'Monthly'
                     when freq_type =  32 then 'Month, rel'
                     when freq_type =  64 then 'Agent start'
                     when freq_type = 128 then 'When idle'
                     else ' -- NULL -- '
                   end as freq_type
                 from sysjobschedules sjs
                 inner join sysschedules ss
                 on sjs.schedule_id = ss.schedule_id) sjsch
...
left outer join ( select job_id, AVG(run_duration) as avg_duration , MIN(run_duration) as min_duration, MAX(run_duration) as max_duration, COUNT(*) as runs
                  from sysjobhistory sjh
				  where step_id = 0
				  group by job_id
				  having COUNT(*) > 0 ) runagg
  on sj.job_id = runagg.job_id
where sj.enabled = 1
order by sj.name
go
select '</table>'
...
/*********************************************************************************************/
/* Trace */
insert into ##MT (trace) values ('Collect SQL Agentlog information')
...
-- Sometimes xp_enumerrorlogs returns CreateDate in nvarchar, if this happens we create another temptable
begin try
...
begin catch
  create table #tbl_enumagentlogs2 (ArchiveNo int, CreateDate nvarchar(50), Size int)
  insert into #tbl_enumagentlogs2 EXEC master.dbo.xp_enumerrorlogs 2;
  insert into #tbl_enumagentlogs select ArchiveNo, convert(datetime,CreateDate,101), Size from #tbl_enumagentlogs2;
  select @LogDate = case
         when @latest_online_check < min(CreateDate) then min(CreateDate)
         else (select max(CreateDate) from #tbl_enumagentlogs where CreateDate < @latest_online_check)
       end
  from #tbl_enumagentlogs
  drop table #tbl_enumagentlogs2;
end catch
...
create table #Mir_Col_Agentlog (LogDate datetime, ErrorLevel nvarchar(3), ErrorText nvarchar(2048))
set @count = 0
while @count < @archno
begin
...
select '<table>'
select '<tr><th>Startup Account</th><th>Max History Rows</th><th>Max Job History Rows</th><th>Idle CPU condition</th><th>Idle CPU Pct</th><th>Idle CPU Duration</th></tr>'
select '<tr>' +
       '<td class="aligncenter" >' + tsgsp.startup_account + '</td>' +
       '<td class="aligncenter" >' + case
                  when tsgsp.jobhistory_max_rows = -1 then 'Unlimited'
                  else CONVERT(varchar,tsgsp.jobhistory_max_rows)
                end + '</td>' +
       '<td class="aligncenter" >' + case
                 when tsgsp.jobhistory_max_rows_per_job = 0 then 'Unlimited'
                 else CONVERT(varchar,tsgsp.jobhistory_max_rows_per_job)
               end + '</td>' +
       '<td class="aligncenter" >' + case
                  when tsgsp.cpu_poller_enabled = 0 then 'Disabled'
                  when tsgsp.cpu_poller_enabled = 1 then 'Enabled'
                  else 'Unknown'
                end + '</td>' +
       '<td class="aligncenter" >' + CONVERT(varchar, tsgsp.idle_cpu_percent) + '</td>' +
       '<td class="aligncenter" >' + CONVERT(varchar, tsgsp.idle_cpu_duration) + '</td>' +
       '</tr>'
from
...
/*********************************************************************************************/
/* Trace */
insert into ##MT (trace) values ('Collect backup information')
...
DECLARE @checkdbThreshold int -- antal dage der må gå mellem succesful checkdb
DECLARE @MaxFullBackupThreshold datetime, @MaxDiffBackupThreshold datetime, @MaxTransactionlogBackupThreshold datetime;

Set @checkdbThreshold = (select convert(int,Value) from #MORLT where Name = 'checkdbThreshold');
Set @MaxFullBackupThreshold = getdate() - (select convert(int,Value) from #MORLT where Name = 'MaxFullBackupThreshold');
Set @MaxTransactionlogBackupThreshold = getdate() - (select convert(int,Value) from #MORLT where Name = 'MaxTransactionlogBackupThreshold')/24;

CREATE TABLE #temp
(ParentObject varchar(100),Object varchar(100),Field varchar(100),Value varchar(100))
 
CREATE TABLE #temp2
(dbname varchar(256),LastSuccesfulCheckDB datetime,TextStatus varchar(100))
...
BEGIN
	...
	INSERT INTO #temp2 (dbname, LastSuccesfulCheckDB, TextStatus)
		(SELECT DISTINCT 
			@name as dbname, 
			Value as LastSuccesfulCheckDB,
			CASE
			   WHEN Value = '1900-01-01 00:00:00.000' 
				THEN CAST('NeverRan' AS VARCHAR)
			   WHEN DATEDIFF(d, Value, GETDATE()) > @checkdbThreshold  
				THEN CAST('NotCurrent' AS VARCHAR)
			   ELSE 
				CAST('IsCurrent' AS VARCHAR)
			END AS TextStatus
		FROM #temp
...
select '<table>'
select '<tr><th>Database Name</th><th>Created</th><th>Recovery Model</th><th>Status</th><th>Latest db backup</th><th>Latest Inc backup</th><th>Latest log backup</th><th>Log Reuse Wait</th><th>Latest Good CheckDb</th></tr>'
...
select '<tr>' +
       ...
       '<td align="left">' + 
                case 
				  ...
				  when a.latest_db_backup < @MaxFullBackupThreshold then '<font color="#FF6600">' + convert(varchar, a.latest_db_backup, 13) + '</font>'
       ...
       '<td align="left">' + 
                case 
				  ...
				  when b.latest_log_backup < @MaxTransactionlogBackupThreshold then '<font color="#FF6600">' + convert(varchar, b.latest_log_backup, 13) + '</font>'
                  else convert(char, b.latest_log_backup, 13) 
                end + '</td>' +
       '<td>' + convert(sysname,c.log_reuse_wait_desc collate database_default) + '</td>' + 
	   '<td ' + case 
	              when t2.TextStatus = 'NeverRan'   then ' class="failed" >- NEVER RAN OR COMPLETED -' + '</td>' 
				  when t2.TextStatus = 'NotCurrent' then ' class="warning" >' + convert(varchar,LastSuccesfulCheckDB) + '</td>' 
				  when t2.TextStatus = 'IsCurrent'  then ' class="fine" >' + convert(varchar,LastSuccesfulCheckDB) + '</td>' 
				  else '>' 
				end +
	            --convert(varchar,LastSuccesfulCheckDB) + '</td>' +
       '</tr>'
...
order by c.name;
select 'NOTE: Backup  colors: Red => Critical! No backup is available, Orange => Backup is possibly obsolete'
select 'NOTE: Checkdb colors: Red => Critical! Check db Never completed or ran with error last time, Orange => Warning! Checkdb is possibly obsolete, Green => OK '
go
...
/*********************************************************************************************/
/* Trace */
insert into ##MT (trace) values ('Collect suspect page information')
...
/*********************************************************************************************/
/* Trace */
insert into ##MT (trace) values ('Collect backup statistics information')
...
/*********************************************************************************************/
/* Trace */
insert into ##MT (trace) values ('Collect database space information')
...
/*********************************************************************************************/
/* Trace */
insert into ##MT (trace) values ('Collect virtual log information')
...
begin
  ...
  where DbName = @name;
  /* Trace */
  insert into ##MT (trace) values (' - Collected virtual log information for: ' + @name)
  fetch dbname_crr into @name, @dbid
end
deallocate dbname_crr
...
select '<tr>' + 
	   ...
	   '<td>' + isnull(convert(varchar,pctLogused), 'NULL') + '</td></tr>'
from #AllDbLogInfo;        
select '</table>'
...
/*********************************************************************************************/
/* Trace */
insert into ##MT (trace) values ('Collect SQL error log information')
...
begin try
  create table #tbl_enumerrorlogs (ArchiveNo int, CreateDate datetime, Size int)
  insert into #tbl_enumerrorlogs EXEC master.dbo.xp_enumerrorlogs 2 
...
begin catch
  create table #tbl_enumerrorlogs2 (ArchiveNo int, CreateDate nvarchar(50), Size int)
  insert into #tbl_enumerrorlogs2 EXEC master.dbo.xp_enumerrorlogs 2 
  insert into #tbl_enumerrorlogs select ArchiveNo, convert(datetime,CreateDate,101), Size from #tbl_enumerrorlogs2
  select @LogDate = case
         when @latest_online_check < min(CreateDate) then min(CreateDate)
         else (select max(CreateDate) from #tbl_enumerrorlogs where CreateDate < @latest_online_check)
       end
...
/*********************************************************************************************/
/* Trace */
insert into ##MT (trace) values ('Summering error log information')
...
/*********************************************************************************************/
/* Trace */
insert into ##MT (trace) values ('Collect trace information')
...
/*********************************************************************************************/
/* Trace */
insert into ##MT (trace) values ('Collect database mirroring information')
...
/*********************************************************************************************/
/* Trace */
insert into ##MT (trace) values ('Collect index fragmentation information')
...
--  if (convert(sysname,DatabasePropertyEX(@dbname,'Status')) != 'OFFLINE') and (@run_defrag_index = 'YES')
    if (@run_defrag_index = 'YES')
      begin
		...
		/* Trace */
        insert into ##MT (trace) values (' - Collected fragmentation information for: ' + @dbname);
		declare @idxcount varchar(30);
		select @idxcount = convert(varchar, count(*)) from #work_to_do where DatabaseID = @dbid
		insert into ##MT (trace) values (' -- ' + @idxcount + ' Index(s) found ');
	  end
	fetch dbname_crr into @dbname, @dbid
end		
...
/*********************************************************************************************/
/* Trace */
insert into ##MT (trace) values ('Collect missing index information')
...
/*********************************************************************************************/
/* Trace */
insert into ##MT (trace) values ('Collect index usage information')
...
/*********************************************************************************************/
/* Trace */
insert into ##MT (trace) values ('Collect statistics information')
...
/*********************************************************************************************/
/* Trace */
insert into ##MT (trace) values ('Collect misc. database information')
...
/*********************************************************************************************/
/* Trace */
insert into ##MT (trace) values ('Collect plan cache information')
...
select '<table>' +
       ...
       '<th>Total MBs - USE Count 1</th><th>Plans - only used once</th><th>Pct of plan type only used once</th></tr>'
...
select '<tr>' +
       ...
       '<td class="aligncenter" >' + CONVERT(varchar,convert(numeric(18,2), 100*CONVERT(numeric(10,2),[Total Plans - USE Count 1]) / CONVERT(numeric(10,2),Total_Plans ))) + '</td>' +
       '</tr>'
from cache
order by Total_Plans desc
select '</table>'

IF EXISTS ( SELECT 1 FROM sys.configurations  WHERE NAME = 'optimize for ad hoc workloads' )
BEGIN
...
Select '<table>'
select '<tr><th>Comment</th><th>Plancache Recommendation</th></tr>'
SELECT '<tr><td>Total cache plan size (MB): ' + cast(@TotalSizeInMB AS VARCHAR(max)) + '. Current memory occupied by adhoc plans only used once (MB):' + cast(@AdHocSizeInMB AS VARCHAR(max)) + '.  Percentage of total cache plan (MB) occupied by adhoc plans only used once :' + cast(CAST((@AdHocSizeInMB / @TotalSizeInMB) * 100 AS DECIMAL(14, 2)) AS VARCHAR(max)) + '%</td> ' AS COMMENTS
        ,'<td>' + CASE 
            ...
            END + '</td></tr>' AS RECOMMENDATIONS
END
select '</table>'
select '<br />'

-- Ref: http://sqlserverperformance.idera.com/memory/optimize-ad-hoc-workloads-option-sql-server-2008/. http://www.sqlskills.com/blogs/kimberly/post/procedure-cache-and-optimizing-for-adhoc-workloads.aspx
...
/*********************************************************************************************/
/* Trace */
insert into ##MT (trace) values ('Collect blocking information')
...
/*********************************************************************************************/
/* Trace */
insert into ##MT (trace) values ('Display activity information')
...
/*********************************************************************************************/
/* Trace */
insert into ##MT (trace) values ('Display IO waits information')
...
/*********************************************************************************************/
/* Trace */
insert into ##MT (trace) values ('Display server wait information')
...
/*********************************************************************************************/
/* Trace */
insert into ##MT (trace) values ('Display performance counter information')
...
/*********************************************************************************************/
/* Trace */
insert into ##MT (trace) values ('Display database property information')
...
/*********************************************************************************************/
/* Trace */
insert into ##MT (trace) values ('Display configuration information')
...
/*****************************************************************************/
/* Trace */
insert into ##MT (trace) values ('Display error log information')
...
:setvar tracefile "MO_trace"
:OUT $(Log_Dir)$(Slash)$(tracefile)$(Dash)$(INSTANCE_NAME)$(Ext)
...
select '<style type="text/css"> 
        ...
        <title>Trace output for ' + convert(sysname,SERVERPROPERTY('ServerName')) + '</title>'
select '</head>';
select '<body>'

select '<table>'
select '<tr><td>' + convert(varchar, stamp, 113) + '</td><td>' + trace + '</td></tr>'
from ##MT
order by stamp
select '</table>'
